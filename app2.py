...
def search():
    ...
    # 🔍 Retrieve relevant document chunks from MongoDB VectorDB
    relevant_chunks = db.query_database(query)
    ...
    # 🤖 Use LLM to generate an answer based on retrieved chunks
    refined_answer = generate_answer_with_llm(query, relevant_chunks)

    return jsonify({
        "query": query,
        "ai_answer": refined_answer,
        "vector_db_output": relevant_chunks  # ✅ Include VectorDB search results
    })
